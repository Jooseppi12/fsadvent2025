...
import { CreateFuncWithArgs, Lazy, MarkResizable, SetOptional, GetOptional, DeleteEmptyFields, Create as Create_1 } from "../WebSharper.Core.JavaScript/Runtime.js"
function isIDisposable(x){
  return"Dispose"in x;
}
export function Search(){
  const searchText=_c.Create_1("");
  const searchFunction=(s) => {
    let elementsToShow;
    if(StartsWith(s, "tag:")){
      const tag=Replace(s, "tag:", "");
      elementsToShow=globalThis.document.querySelectorAll((((_2) =>(_3) => _2(".bookmark:has(a[data-tags*=\""+toSafe(_3)+"\"])"))((x) => x))(tag));
    }
    else elementsToShow=globalThis.document.querySelectorAll((((_2) =>(_3) => _2(".bookmark:has(a[href*=\""+toSafe(_3)+"\"])"))((x) => x))(s));
    globalThis.document.querySelectorAll(".bookmark").forEach(CreateFuncWithArgs((_2) => _2[0].classList.add("hidden")), null);
    elementsToShow.forEach((_2) => {
      _2.classList.remove("hidden");
    }, null);
  };
  const this_1=new ProviderBuilder("New_1");
  const t=(this_1.h.push(new VarStr("searchtext", searchText)),this_1);
  const b=(t.h.push(EventQ2(t.k, "search", () => t.i, () => {
    Get(searchFunction, searchText.View);
  })),t);
  const p=CompleteHoles(b.k, b.h, [["searchtext", 0, null]]);
  const i=new TemplateInstance(p[1], search(p[0]));
  let _1=(b.i=i,i);
  return _1.Doc;
}
class Object_1 {
...
function Replace(subject, search_1, replace){
  function replaceLoop(subj){
    const index=subj.indexOf(search_1);
    if(index!==-1){
      const replaced=ReplaceOnce(subj, search_1, replace);
...
function ReplaceOnce(string_1, search_1, replace){
  return string_1.replace(search_1, replace);
}
...
class HashSet extends Object_1 {
  ...
  GetEnumerator(){
    return Get_1(concat_3(this.data));
  }
  ExceptWith(xs){
    const e=Get_1(xs);
    try {
  ...
  return{GetEnumerator:() => {
    ...
    return new T(e1, null, (x) => {
      if(x.s.MoveNext()){
        ...
        x.s=null;
        return first[0]&&(first[0]=false,x.s=Get_1(s2),x.s.MoveNext()?(x.c=x.s.Current,true):(x.s.Dispose(),x.s=null,false));
      }
    }, (x) => {
      const x_1=x.s;
      if(!Equals(x_1, null))x_1.Dispose();
        ...
        if(value[1].$1.$==1){
          if(value[1].$1.$1.$==1){
            if(value[1].$1.$1.$1.$==0){
              ...
              const s_2=value[0];
              _9=Dynamic_1(attrName, Map((vs) => s_2+concat("", vs), Sequence(value[1])));
            }
          }
          else {
            const s_3=value[0];
...
function RenderedFullDocTemplate(){
  return _c_3.RenderedFullDocTemplate;
}
function set_RenderedFullDocTemplate(_1){
  _c_3.RenderedFullDocTemplate=_1;
}
class View { }
...
class KeyCollection extends Object_1 {
  d;
  GetEnumerator(){
    return Get_1(map((kvp) => kvp.K, this.d));
  }
  constructor(d){
    super();
    this.d=d;
...
class Elt_1 extends TemplateHole {
  ...
  WithName(n){
    return new Elt_1(n, this.fillWith);
  }
  constructor(name, fillWith){
    super();
    this.name=name;
...
class ValueCollection extends Object_1 {
  d;
  GetEnumerator(){
    return Get_1(map((kvp) => kvp.V, this.d));
  }
  constructor(d){
    super();
    this.d=d;
